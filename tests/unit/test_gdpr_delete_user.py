...
def _event(user_id: str, caller_id: str) -> dict:
    return {
        ...
        "requestContext": {"requestId": "req-gdpr", "authorizer": {"user_id": caller_id}},
...
def test_self_erasure_marks_user_deleted(dynamodb_tables, mock_settings: Settings):
    user_repo.create_user(mock_settings, "usr_erase1", "Erase Me", "erase@example.com", "TestPass123!")
...
def test_also_soft_deletes_user_images(dynamodb_tables, mock_settings: Settings):
    user_repo.create_user(mock_settings, "usr_erase2", "With Images", "wi@example.com", "TestPass123!")
